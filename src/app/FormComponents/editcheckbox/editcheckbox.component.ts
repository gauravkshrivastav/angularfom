...
@Component({
  selector: 'app-editcheckbox',
  templateUrl: './editcheckbox.component.html',
  styleUrls: ['./editcheckbox.component.scss']
})
export class EditcheckboxComponent {
  ...
  ngOnInit() {
    ...
    if (localStorage.getItem("checkboxdata")) {
      var button1: any = localStorage.getItem("checkboxdata");
      this.checkboxdatas = JSON.parse(button1);
      console.log("checkboxdatas", this.checkboxdatas);
      ...
      this.registerForm = this.formBuilder.group({
        ...
        text3: [''],
        text4: [''],
        addbutton:[this.boxshadowValue(),[Validators.required]],
  ...
  boxshadowValue(){
    if(this.checkboxdatas.addbutton=="Yes"){
      this.boxshadowchecked="checked";
      return "Yes";
    }else if(this.checkboxdatas.addbutton=="No"){
  ...
  reloadComponent(self: boolean, urlToNavigateTo ?: string){
    ...
    const url = self ? this.router.url : "component/checkbox";
